/**
 * OBSIDIAN Ghost Share — App.tsx (Ritual Chain on-chain)
 */
...
export default function App() {
  ...
  const initIdentity = async (forceRegen = false) => {
    try {
      if (!forceRegen) {
        ...
        // Validate keys are secp256k1 (base64 of 65 bytes = 88 chars for pub)
  ...
  useEffect(() => {
    ...
    const handleChainChange = (chainIdHex: string) => {
    ...
    const handleAccountsChange = (accounts: string[]) => {
      if (accounts.length === 0) {
        ...
        setRitualSigner(null); setRitualProvider(null);
      } else {
  ...
  const handleConnectWallet = async () => {
    try {
      ...
      appendLog("wallet", `CONNECTED: ${address.slice(0, 10)}...${address.slice(-6)}`);
      if (chainId !== RITUAL_CHAIN.id) {
        appendLog("wallet", "WRONG CHAIN — SWITCHING TO RITUAL...");
        await switchToRitual();
        const net = await provider.getNetwork();
        const id  = Number(net.chainId);
        setWalletState(prev => ({ ...prev, chainId: id, onRitual: id === RITUAL_CHAIN.id }));
        appendLog("wallet", id === RITUAL_CHAIN.id ? "✓ RITUAL CHAIN ACTIVE" : "SWITCH FAILED — ADD RITUAL MANUALLY");
      } else {
  ...
  // ── GHOST SHARE: full Ritual on-chain pipeline ────────────
  const executeGhostTransmission = async (
    ...
    try {
      ...
      // Guard: wallet must be connected & on Ritual
      if (!walletState.connected || !walletState.onRitual || !ritualSigner || !ritualProvider) {
        // Fallback: run local ECIES encryption (no on-chain tx)
        appendLog("system", "WALLET NOT ON RITUAL — RUNNING LOCAL ECIES ENCRYPTION", true);
        appendLog("crypto", "GENERATING EPHEMERAL secp256k1 KEYPAIR...");
        const { sealedPayload, senderEphemeralPrivateKeyBase64 } =
          await encryptSecret(recipientPubKey || (identity?.publicKeyBase64 ?? ""), secret, expOption);
        ...
        setLastTxHash(null);
        appendLog("system", "LOCAL ENCRYPTION COMPLETE (CONNECT WALLET FOR ON-CHAIN TX)", true);
      ...
      // Build sealed payload with Ritual on-chain fields
      const { sealedPayload } = await encryptSecret(
        identity?.publicKeyBase64 ?? recipientPubKey,
        secret,
        expOption
      );
      ...
      appendLog("network", `SEALED PAYLOAD: ${serialized.slice(0, 30)}...`);
      appendLog("network", `EXPIRY: ${expOption.toUpperCase()}`);
  ...
  const executeRevealRecovery = async (
    ...
    try {
      ...
      const queue = [
        ...
        { tag: "crypto",  text: "CALCULATING SHARED SECRET MATRIX (ECDH secp256k1 → HKDF-SHA256)", delay: 350, stage: ExecutionStage.ENCRYPT },
          ...
          if (idx === queue.length - 1) {
            setTimeout(async () => {
              try {
                ...
                const decrypted = await decryptSecret(sealedPayload, recipientPrivKey.trim());
  ...
  return (
    <div className="h-screen w-screen flex flex-col font-sans bg-zinc-950 text-zinc-100 overflow-hidden relative">
      ...
      <div className="lg:hidden shrink-0 border-b border-zinc-900 bg-zinc-950/80 p-4.5 flex items-center justify-between z-40 select-none">
        <button onClick={() => { setMobileShowLeft(!mobileShowLeft); setMobileShowRight(false); }} className="text-zinc-400 hover:text-white p-1 hover:bg-zinc-900/40 rounded transition-colors">
        ...
        <button onClick={() => { setMobileShowRight(!mobileShowRight); setMobileShowLeft(false); }} className="text-zinc-400 hover:text-white p-1 hover:bg-zinc-900/40 rounded transition-colors flex items-center space-x-1.5">
